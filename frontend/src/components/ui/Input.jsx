...
const Input = forwardRef(({
  ...
  return (
    <div className={`flex flex-col gap-1.5 w-full ${className} group`}>
      {label && (
        <label htmlFor={id} className="text-xs font-mono uppercase tracking-widest text-text-secondary group-focus-within:text-brand transition-colors select-none">
          {label}
      ...
      {error && (
        <span id={`${id}-error`} className="text-xs text-red-500 font-mono mt-1 flex items-center gap-1">
